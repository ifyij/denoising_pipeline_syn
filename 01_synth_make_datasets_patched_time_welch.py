...
class Config:
    ...
    # Target is the latent Gaussian/shoulder signal plus a reduced smooth floor.
    # This keeps the physically important peak and integrated fluctuation level
    # available to the trainer instead of asking the CNN to suppress everything.
    target_floor_frac_of_obs: float = 0.30
    ...
    preserve_signal_floor_fraction: float = 0.02
...
def make_noisy_sample_from_time_domain(
    ...
    _, y_target_welch = compute_psd_welch(x_target_ts, cfg)

    # The train target should represent the recoverable underlying spectrum:
    # keep the known latent Gaussian/shoulder shape, plus a reduced smooth floor.
    # The finite-Welch target is still used as a gentle roughness carrier, but it
    # is not allowed to pull the target peak/area far away from the latent signal.
    y_signal_floor = max(float(np.max(y_latent_welch)) * cfg.preserve_signal_floor_fraction, cfg.min_noise_floor_v2hz)
    y_floor_target = np.maximum(y_target_floor_model, y_signal_floor)
    y_target_model = np.maximum(y_latent_welch + y_floor_target, cfg.min_noise_floor_v2hz)
    rough_ratio = np.clip(
        y_target_welch / np.maximum(y_target_model, cfg.min_noise_floor_v2hz),
        0.75,
        1.25,
    )
    y_target = np.maximum(y_target_model * rough_ratio, cfg.min_noise_floor_v2hz)
