...
- Loads 24 CECE channels from per-channel .xlsx files
- Builds 24 pair slots arranged on a 4×6 grid using row-neighbor logic:
    col 1: (ch1,ch2), col 2: (ch2,ch3), ..., col 5: (ch5,ch6), col 6: (ch5,ch6)
  repeated for each row of 6 channels so the grid has 24 occupied slots
- Computes for each pair:
    * auto PSD of channel a
    * auto PSD of channel b
    * cross-power Sxy
- Denoises |Sxy| with the trained CNN and reattaches the original phase:
    Sxy_denoised = denoise(|Sxy|) * exp(1j * angle(Sxy))
- Produces:
    * 4×6 grid: reference autopower for each pair = sqrt(Sxx*Syy)
    * 4×6 grid: raw |Sxy|
    * 4×6 grid: CNN-denoised |Sxy|
    * 24 individual pair plots with three curves each:
          sqrt(Sxx*Syy), |Sxy|, |Sxy_denoised|
- Optionally saves CSVs, figures, and metadata

Why denoise the magnitude only?
- The CNN was trained on real nonnegative PSD targets, not complex spectra.
- Denoising |Sxy| keeps the model compatible with training.
- Reattaching the original phase preserves phase information for downstream use.
...
class RunConfig:
    ...
    # ---- Pairing layout ----
    # neighbor_row_repeat gives 24 grid slots from 24 channels:
    # 5 unique neighbor pairs per row + repeat last pair in col 6.
...
def plot_grid_pair_metric(
    data_by_slot: List[Tuple[str, np.ndarray, np.ndarray]],
    xlim_hz=None,
    title_prefix="",
    suptitle="",
    ylabel="",
...
def plot_individual_pair_comparison(
    label: str,
    f: np.ndarray,
    auto_ref: np.ndarray,
    cross_mag: np.ndarray,
    den_mag: np.ndarray,
    xlim_hz=None,
    figsize=(9, 5),
    yscale: str = "linear",
    ...
    ax.set_title(label if str(yscale).lower() != "log" else f"{label} (log-lin)")
    ax.grid(True, which="both")
    ax.legend()
    fig.tight_layout()
    return fig
...
def build_trained_model_from_ckpt_cfg(cfg: dict) -> CNNDenoiser1D:
    ...
    return CNNDenoiser1D(base_channels=base_channels, kernel_size=kernel_size, dropout=dropout)
...
def denoise_positive_spectrum(
    ...
    if normalize_mode == "per_sample":
        x_norm, mu, sig = normalize_per_sample(spec, eps=eps)
        xb = torch.from_numpy(x_norm.astype(np.float32))[None, None, :].to(device)
        y_norm = model(xb).detach().cpu().numpy()[0, 0, :].astype(np.float64)
        return np.maximum(y_norm * sig + mu, 0.0)

    if normalize_mode == "global":
        x_norm, mu, sig = normalize_global(spec, stats=(normalize_stats or {}), eps=eps)
        xb = torch.from_numpy(x_norm.astype(np.float32))[None, None, :].to(device)
        y_norm = model(xb).detach().cpu().numpy()[0, 0, :].astype(np.float64)
        return np.maximum(y_norm * sig + mu, 0.0)

    if normalize_mode in ("none", "raw"):
        xb = torch.from_numpy(spec.astype(np.float32))[None, None, :].to(device)
...
@torch.no_grad()
def compute_cross_and_denoise(
    model: nn.Module,
    channels: Dict[str, np.ndarray],
    pair_slots: List[Tuple[str, str]],
    fs: float,
    normalize_mode: str,
...
def main():
    ...
    fig_auto = plot_grid_pair_metric(
        auto_ref_slot,
        xlim_hz=CFG.xlim_hz,
        title_prefix=CFG.title_prefix,
        suptitle="Pair Reference Autopower sqrt(Sxx*Syy)",
        ylabel="sqrt(Sxx*Syy)",
        figsize=CFG.figsize_grid,
    )
    fig_cross = plot_grid_pair_metric(
        cross_slot,
        xlim_hz=CFG.xlim_hz,
        title_prefix=CFG.title_prefix,
        suptitle="Raw Cross-power |Sxy|",
        ylabel="|Sxy|",
        figsize=CFG.figsize_grid,
    )
    fig_den = plot_grid_pair_metric(
        den_slot,
        xlim_hz=CFG.xlim_hz,
        title_prefix=CFG.title_prefix,
        suptitle="CNN Denoised Cross-power |Sxy|",
        ylabel="|Sxy| denoised",
        figsize=CFG.figsize_grid,
    )
    ...
    for rec in results:
        # ----- Linear plot -----
        fig = plot_individual_pair_comparison(
            label=rec["label"],
            f=rec["f"],
            auto_ref=rec["auto_ref"],
            cross_mag=rec["cross_mag"],
            den_mag=rec["den_mag"],
            xlim_hz=CFG.xlim_hz,
            figsize=CFG.figsize_pair,
            yscale="linear",
        )
        if CFG.save_figs:
            fig.savefig(
                outdir / "pair_figs" / f"{rec['label']}__comparison.png",
                dpi=CFG.fig_dpi,
            )
        plt.close(fig)

        # ----- Log-Lin plot -----
        fig_log = plot_individual_pair_comparison(
            label=rec["label"],
            f=rec["f"],
            auto_ref=rec["auto_ref"],
            cross_mag=rec["cross_mag"],
        ...
        if CFG.save_figs:
            ...
            fig_log.savefig(
                outdir / "pair_figs_loglin" / f"{rec['label']}__comparison_loglin.png",
                dpi=CFG.fig_dpi,
            )
        plt.close(fig_log)
